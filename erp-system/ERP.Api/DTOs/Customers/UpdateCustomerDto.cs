...
public sealed class UpdateCustomerDtoValidator : AbstractValidator<UpdateCustomerDto>
{
    ...
    public UpdateCustomerDtoValidator(ApplicationDbContext context, IHttpContextAccessor httpContextAccessor)
    {
        _context = context;
    ...
    {
        // Get the current customer ID from route parameter
        var httpContext = _httpContextAccessor.HttpContext;
        if (httpContext?.Request.RouteValues.TryGetValue("id", out var idValue) == true && 
            long.TryParse(idValue?.ToString(), out var customerId))
        {
            return !await _context.Customers.AnyAsync(c => c.Email.ToLower() == email.ToLower() && c.Id != customerId, cancellationToken);
        }
    ...
    private async Task<bool> BeUniquePhone(string phone, CancellationToken cancellationToken)
    {
        ...
        if (httpContext?.Request.RouteValues.TryGetValue("id", out var idValue) == true && 
            long.TryParse(idValue?.ToString(), out var customerId))
        {
            return !await _context.Customers.AnyAsync(c => c.Phone == phone && c.Id != customerId, cancellationToken);
        }
